...
static void `$INSTANCE_NAME`_ProcessTxData(uint8 socket, uint16 offset, uint8* buffer, uint16 length)
{
	...
	/*
	 * Read the offset pointer, and calculate the base address for the start of write in to the chip buffers.  Written data will be offset by the offset byte count specified in the parameters.
	Читаем смещение указателя, и рассчитать базовый адрес для начала записи в   буфер  чипа. Письменные данные будут компенсированы смещения счетчик байтов, указанных в параметрах
	 */
...
static void `$INSTANCE_NAME`_ProcessRxData(uint8 socket, uint16 offset, uint8* buffer, uint16 length, uint8 flags)
{
	...
	/*
	 * Read the offset pointer, and calculate the base address for the start of read in to the chip buffers.  The data read will be offset by the offset byte count	specified in the parameters.
	   Читать смещение указателя, и рассчитать базовый адрес  начала в буфере чипа. Считанные данные будут смещены  на счетчик байтов, указанных в параметрах
	 */
	base = `$INSTANCE_NAME`_GetSocketRxReadPtr(socket) + offset;
	PointerOffset = (base & 0x07FF);
	addr = PointerOffset + `$INSTANCE_NAME`_SOCKET_RX_BASE(socket);
	/* рассчитать количество байтов из указателя на конец буфера         calculate the number of bytes from the pointer to the end of the buffer   */
	size = 0x0800 - PointerOffset;
	/*
	 * is there enough space to read the complete packet or, should the data be split in to two reads.
	    есть достаточно места, чтобы прочитать полный пакет или, если данные были разделена на две операции чтения
	 */
	if ( length > size) {
		/*
		 * There is more data to send than will fit from the pointer to the end	 of the memory, so we will write the data in two chunks.  First, write the
		 * data from the pointer to the end of the buffer, then write the portion	 remaining to the start (base ptr) of the socket buffer
		   данных больше ,   чем может поместиться от указателя до конца памяти, поэтому мы будем писать данные в двух кусков. Во-первых, написать
данные из указателя на конец буфера, а затем написать часть оставшегося на начало (база PTR) буфера сокета
		 */
	...
	if (flags & 0x01) {
		...
		base += length;
		/* 
		 * Store the new write pointer so that the device knows that there is data waiting to be transmitted over the link.
		сохранить новый указатель, чтоб устройство знало, что имеются данные, ожидая которые должны быть переданы по каналу
		 */
