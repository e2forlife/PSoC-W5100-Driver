...
#define ETH_READ_OP         ( 0x0F )
#define ETH_WRITE_OP        ( 0xF0 )
...
#define ETH_PROTO_TCP        ( 1 )
#define ETH_PROTO_UDP        ( 2 )
#define ETH_PROTO_IP         ( 3 )
#define ETH_PROTO_MAC        ( 4 )
 ...
 *
 * \note this function is usuallt called from ETH_Start()
 * \sa ETH_Start()
 */
void ETH_Init(uint8* mac, uint32 ip, uint32 subnet, uint32 gateway);
 ...
 * \sa ETH_PROTO_MAC
 */
uint8 ETH_SocketOpen( uint8 Protocol, uint16 port, uint8 flags );
 ...
 * has no effect.
 */
void ETH_SocketClose( uint8 socket );
 ...
 */
uint8 ETH_SocketProcessConnections( uint8 socket );
 ...
 */
uint8 ETH_SocketEstablished( uint8 socket );
 ...
 */
uint16 ETH_SocketRxDataWaiting( uint8 socket );
 ...
 * \sa ETH_SocketClose()
 */
void ETH_TcpStartServer( uint8 socket );
 ...
 * \sa ETH_TcpStartServer()
 * \sa ETH_TcpOPen();
 * \sa ETH_SocketClose()
 */
void ETH_TcpStartServerWait( uint8 socket );
 ...
 * \sa ETH_TcpOpen()
 * \sa ETH_TcpStartServer()
 * \sa ETH_TcpStartServerWait()
 * \sa ETH_TcpConnected()
 * \sa ETH_TcpDisconnect()
 */
void ETH_TcpConnect( uint8 socket, uint32 ip, uint16 port );
 ...
 * \sa ETH_TcpStartServer()
 */
uint8 ETH_TcpConnected( uint8 socket );
 ...
 * \sa ETH_TcpStartServerWait()
 */
void ETH_TcpDisconnect( uint8 socket );
 ...
 * \sa ETH_TcpReceive()
 */
uint16 ETH_TcpSend(uint8 socket, uint8* buffer, uint16 len);
 ...
 * \sa ETH_TcpSend()
 */
uint16 ETH_TcpReceive( uint8 socket, uint8* buffer, uint16 length );
 ...
 *
 * this function is a shortcut to using the ETH_TcpSend()
 ...
 * \sa ETH_TcpReceive()
 */
void ETH_TcpPrint( uint8 socket, const char* str );
 ...
 * \sa ETH_UdpReceive()
 */
uint16 ETH_UdpSend(uint8 socket, uint32 ip, uint16 port, uint8* buffer, uint16 length);
 ...
 * \sa ETH_UdpSend()
 */
uint16 ETH_UdpReceive(uint8 socket, uint32 *ip, uint16 *port, uint8* buffer, uint16 length);
