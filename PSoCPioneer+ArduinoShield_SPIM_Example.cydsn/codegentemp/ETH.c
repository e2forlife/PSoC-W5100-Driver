...
#if (1 == 1) // W5100 Specific
const uint16 ETH_SOCKET_TX_BASE[4] = { 0x4000, 0x4800, 0x5000, 0x5800 };
const uint16 ETH_SOCKET_RX_BASE[4] = { 0x6000, 0x6800, 0x7000, 0x7800 };
...
#elif (1 == 2) // W5200 Specific
	
const uint16 ETH_SOCKET_TX_BASE[8] = { 0x8000, 0x8800, 0x9000, 0x9800, 0xA000, 0xA800, 0xB000, 0xB800 };
const uint16 ETH_SOCKET_RX_BASE[8] = { 0xC000, 0xC800, 0xD000, 0xD800, 0xE000, 0xE800, 0xF000, 0xF800 };
 ...
 */
#define ETH_SpiDone     (SPI0_ReadTxStatus() & (SPI0_STS_SPI_DONE | SPI0_STS_SPI_IDLE))
/* ------------------------------------------------------------------------ */
/**
 ...
 */
void 
ETH_ChipWrite(uint16 addr, uint8 *dat, uint16 length)
{
	uint16 txLen;
	uint16 txCount;
	uint16 adr;
	...
	do {
	     ...
	     */
		SPI0_WriteTxData(ETH_WRITE_OP);
		SPI0_WriteTxData((adr>>8)&0x00FF);
		SPI0_WriteTxData(adr&0x00FF);
		txLen = 1;
#elif (1 == 2)
	    /*
		...
		SPI0_WriteTxData( (adr>>8) & 0x00FF);
		SPI0_WriteTxData( adr & 0x00FF );
		txLen = (lenght > ETH_BURST_MAX)? ETH_BURST_MAX : length;
		SPI0_WriteTxData( 0x80 | ((txLen>>8)&0x007F) );
		SPI0_WriteTxData( txLen & 0x00FF );
#endif
		/*
		...
		// Transmit data block
		do {
			while ( (SPI0_ReadTxStatus() & SPI0_STS_TX_FIFO_NOT_FULL) == 0);
			SPI0_WriteTxData(dat[txCount++]);
			--length;
			--txLen;
			++adr;
		} while ( txLen > 0 );
 ...
 */
void
ETH_ChipRead(uint16 addr, uint8 *dat, uint16 length)
{
	uint16 rxIndex;
	uint16 address;
	uint16 rxLen;
	...
	do {
		...
		SPI0_WriteTxData(ETH_READ_OP);
		SPI0_WriteTxData(address>>8);
		SPI0_WriteTxData(address&0x00FF);
		...
		SPI0_WriteTxData( address>>8);
		SPI0_WriteTxData( address & 0x00FF );
		SPI0_WriteTxData( 0x7F & (rxLen>>8) );
		SPI0_WriteTxData( rxLen&0x00FF );
#endif
...
#define ETH_SpiDone    ((SPI0_SpiUartGetTxBufferSize()==0)?1:0)
/* ------------------------------------------------------------------------ */
/* ------------------------------------------------------------------------ */
/**
 * \brief write Data to the W5100 at the specified address
 ...
 */
void ETH_ChipWrite16( uint16 addr, uint16 val )
{
	uint16 bigVal;
 ...
 */
uint16 ETH_ChipRead16( uint16 addr )
{
	uint16 val;
